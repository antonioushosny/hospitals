              ...
              <div class="col-lg-9">
                ...
                <div class="form-group row">
                  ...
                  <div class="col-md-9 col-form-label">
                    @php
                      $status = old('countries_status', isset($country) ? $country->countries_status : 'active');
                    @endphp
                    <div class="form-check form-check-inline mr-1">
                      <input class="form-check-input" id="active" type="radio" value="active" name="countries_status" {{ $status == 'active' ? 'checked' : '' }}>
                      <label class="form-check-label" for="active">{{ __('admin::lang.active') }}</label>
                    </div>
                    <div class="form-check form-check-inline mr-1">
                      <input class="form-check-input" id="stopped" type="radio" value="stop" name="countries_status" {{ $status == 'stop' ? 'checked' : '' }}>
            ...
            <div class="tab-pane fade {{ $activeLocale == $lang->locale ? 'show active' : '' }}" id="{{ $lang->locale }}" role="tabpanel" aria-labelledby="{{ $lang->locale }}-tab">
              ...
              <div class="row">
                ...
                <div class="col-lg-9">
                  ...
                  <div class="form-group row"  id="{{ $lang->locale }}-countries_title" >
                    <label class="col-md-3 col-form-label" for="countries_title">{{ __('admin::lang.countries_title') }}<span class="text-danger"> *</span></label>
                    <div class="col-md-9">
                    
                      <input class="form-control {{ $errors->first($lang->locale .'.countries_title') ? 'is-invalid' : '' }}" id="{{ $lang->locale .'[countries_title]' }}" type="text" name="{{ $lang->locale .'[countries_title]' }}" placeholder="{{ __('admin::lang.countries_title') }}" >

                      @if ($errors->first($lang->locale .'.countries_title'))
                        <div class="invalid-feedback">{{ $errors->first($lang->locale .'.countries_title') }}</div>
                      @endif
                    </div>
                  </div>
